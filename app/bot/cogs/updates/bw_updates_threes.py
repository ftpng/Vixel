...
class BedwarsUpdatesThrees(commands.Cog):
    ...
    async def update_message(self):
        try:
            for lb_key, lb_info in BEDWARS_LEADERBOARDS_THREES.items():
                ...
                if changes_list:
                    ...
                    if channel:
                        header = f"**Threes {lb_info['display_name']} Updates**\n"
                        max_length = 2000
                        chunk = header
            ...
            import traceback
            print(f"An error occurred in leaderboard Threes updates: {e}")
            traceback.print_exc()
...
async def setup(client: commands.Bot):
    await client.add_cog(BedwarsUpdatesThrees(client))
